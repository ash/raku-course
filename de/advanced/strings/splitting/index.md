...
Die Methode `words` teilt einen String in eine Liste seiner durch Leerzeichen getrennten Woerter auf:
...
Ein wichtiger Sonderfall ist das Aufteilen am **leeren String** `''`: Dabei wird ein String in seine einzelnen Zeichen zerlegt, aber es wird auch am Anfang und am Ende jeweils ein leerer String eingefuegt, sodass das Ergebnis zwei Elemente mehr hat, als man erwarten wuerde:
...
In die andere Richtung fuegt die Methode `join` eine Liste von Werten zu einem einzelnen String zusammen und setzt dabei ein Trennzeichen dazwischen:

```raku
say <a b c>.join('-'); # a-b-c
```

Zwei weitere Methoden arbeiten mit Teilen eines Strings. Die Methode `substr` extrahiert ein Stueck, basierend auf einer Startposition und optional einer Laenge:
