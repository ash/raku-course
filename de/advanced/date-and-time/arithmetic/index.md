...
Daten unterstützen Arithmetik, was viele alltägliche Berechnungen einfach macht.
...
Wenn du ein Datum von einem anderen subtrahierst, erhältst du die Anzahl der Tage zwischen ihnen:

```raku
say Date.new(2026, 7, 4) - Date.new(2026, 6, 27); # 7
```

Für größere Schritte akzeptieren die Methoden `later` und `earlier` benannte Argumente wie `:days`, `:months` oder `:years`:
